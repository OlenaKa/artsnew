...
function getPriceNet(surface: number): number {
  if (surface < 2) {
    return surface * priceTill2M2;
  } else if (surface >= 2 && surface < 4) {
    return surface * price2to4M2;
  } else if (surface >= 4 && surface < 10) {
    return surface * price4to10M2;
  } else if (surface >= 10 && surface < 20) {
    return surface * price10to20M2;
  } else if (surface >= 20 && surface < 50) {
    return surface * price20to50M2;
  } else {
    return surface * priceAbove50M2;
  }
}
type PriceDetails = {
  priceNet: number | string;
  pricePerPc: number | string | null;
